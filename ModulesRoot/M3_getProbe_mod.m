function [Datastr] = M3_getProbe_mod(Datastr,probefolder,probeMarkerLocal,idxProbe,clusterCell,probeCell)
...
% Example :
% [20 21 23 19 24];
% [24 25 27 23 28]; with middle=[24 25 27 23 26 28];
% Where 20=lower left, 21=lower right 23=upper left 19=upper right, 24=tip
% middle!
% Where 24=lower left, 25=lower right 27=upper left 23=upper right, middle=26 28=tip
% 
...
% 
% NOTES)
% Modification is made on the number of need LEDs when derivind
% anatomicallandmarks! It was used to reduce from 3 to 2 given quality of
% recordings.

%% Get info

mTrans = Datastr.Marker.MarkerDataTransform;
mTrans2 = Datastr.Marker.MarkerDataTransform2;
rotax = Datastr.Marker.MarkerDataTransformAxis;
rotang = Datastr.Marker.MarkerDataTransformAngles;
...
for i = 1:length(clusterCell)
    ...
    for j = 1:length(probePerCluster)
        if ~isempty(probePerCluster{j})
            ...
            [C3DProc] =  procC3D_mod(C3DRaw,mTrans,mTrans2,rotax,rotang); % Mind that this interpolates data
