function [Datastr] = M3_getProbeMat(Datastr,probefolder,probeMarkerLocal,idxProbe,clusterCell,probeCell)
% gBMPDynUI probeFolder=1; pMarkLocal=5; idxProbe=1; clusterCell=3; probeCell=10;
% 
% NOTE: This does the same as M3_getProbe, except that it loads the probe
% data from a .mat file (in case exporting to C3D was not an option).
% 
...
%             [C3DRaw] = readC3D(FullFileName);
            FullFileName = [rootfolder '\' probefolder '\' probePerCluster{j} '.mat']; 
            load(FullFileName); % Loads C3DRaw when obtained from getVZProjData
