function [C3Ddata] = getProbeGlobalParf(C3Ddata,Body)
%% getProbeGlobal
...
%     h_wait = waitbar(0);  % Fancy waitbar
    for iPos = 1:length(Body.ProbePos)
        ...
        if size(clusterMarkerGlobal,2) > 3
            ...
            % Do a final check on if there are 3 markers (allows you to continue with less than 3 markers, but data will be wrong)
            % You can't make the data NaN, as the SVD cannot handle this. Instead it's made zero.
            hasThree = sum(usedMrk,2) >= 3;
            usedMrk(~hasThree,:) = true;
            clusterMarkerGlobal(~hasThree,:) = 0;
...
%         waitbar(iPos/length(Body.ProbePos),h_wait,['Constructing ' posName]);
        
        % Reconstruct
        dumVar = zeros(size(clusterMarkerGlobal,1),3);
        parfor idx = 1:size(C3Ddata.Marker.MarkerData,1)
