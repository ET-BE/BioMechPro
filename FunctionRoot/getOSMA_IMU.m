function [] = getOSMA_IMU(osinstallpath,subjosmod,subjosmaset,subjosikmot)
%% getOSMA_IMU
...
% The .sto file is contained in a MA foler in the IMU folder.
...
if ~isempty(strfind(subjosikmot,'\'))
    ...
    stofilepath = subjosikmot(1:foo(end));
else
    stofilepath = pwd;
end
...
if ~isempty(strfind(subjosikmot,'\'))
     pathinfo = strsplit(subjosikmot,'\');
    %pathinfo{end-2} assumed to be the subject and pathinfo{end2} file
    file=pathinfo{end};
    split=strsplit(file,'.');
    filename=split{1};
    genfilename = [pathinfo{end-2} filename(end-2:end)]; %subject and 3 last digits from filename, assumed to be the trial number
else
    genfilename = regexprep(subjosikmot,'.mot','');
end
...
% Send system command to perform MA using specific settings
