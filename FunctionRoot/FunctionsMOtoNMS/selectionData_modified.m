function [DataSelected,time] = selectionData_modified(filtData,AnalysisWindow,Rate,offset,flag)
...
if flag %run as the original function. Using an analysis window
    if nargin<4
        offset=0;
    end
...
else %(flag==0) as an temporary alternative function, no analysis window
    for k=1:length(filtData)
        startFrame=1;
        endFrame= length(filtData{k}); %n of frames, equivalent to size(filtData{k},1)
        
        timeStartFrame=(startFrame-1)/Rate;
        timeEndFrame=(endFrame-1)/Rate;
        
        time{k}=[timeStartFrame: 1/Rate: timeEndFrame]';
    end
    DataSelected=filtData;
